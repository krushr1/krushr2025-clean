...
import {
  ...
  useId
} from "/chunks/chunk-YDVM2LK4.js";
import {
  FloatingInput
} from "/chunks/chunk-K6PDETW2.js";
import {
  useOptimisticAction
...
function SortableChecklistItem({
  ...
  const [editText, setEditText] = (0, import_react9.useState)(item.text);
...
function TaskChecklist({
  ...
  const [newChecklistTitle, setNewChecklistTitle] = (0, import_react9.useState)("");
  const [newItemTexts, setNewItemTexts] = (0, import_react9.useState)({});
  const [expandedChecklists, setExpandedChecklists] = (0, import_react9.useState)(/* @__PURE__ */ new Set());
  const [showNewChecklist, setShowNewChecklist] = (0, import_react9.useState)(false);
  const newChecklistRef = (0, import_react9.useRef)(null);
...
function TaskDetail({ taskId, open, onClose, onUpdate }) {
  const [comment, setComment] = (0, import_react10.useState)("");
  const [editingTitle, setEditingTitle] = (0, import_react10.useState)(false);
  const [editingDescription, setEditingDescription] = (0, import_react10.useState)(false);
  const [tempTitle, setTempTitle] = (0, import_react10.useState)("");
  const [tempDescription, setTempDescription] = (0, import_react10.useState)("");
  const [showEditModal, setShowEditModal] = (0, import_react10.useState)(false);
  const { data: task, refetch } = trpc.task.get.useQuery(
    { id: taskId },
    {
      enabled: !!taskId,
...
function KanbanBoard({ kanban, className }) {
  const [activeTask, setActiveTask] = (0, import_react11.useState)(null);
  const [searchTerm, setSearchTerm] = (0, import_react11.useState)("");
  const [showFilters, setShowFilters] = (0, import_react11.useState)(false);
  const [selectedPriorities, setSelectedPriorities] = (0, import_react11.useState)([]);
  const [selectedAssignees, setSelectedAssignees] = (0, import_react11.useState)([]);
  const [showCreatePanel, setShowCreatePanel] = (0, import_react11.useState)(false);
  const [selectedTaskId, setSelectedTaskId] = (0, import_react11.useState)(null);
  const [selectedColumnId, setSelectedColumnId] = (0, import_react11.useState)(null);
  const [showColumnManagement, setShowColumnManagement] = (0, import_react11.useState)(false);
  const [editingColumn, setEditingColumn] = (0, import_react11.useState)(null);
  const [newColumnTitle, setNewColumnTitle] = (0, import_react11.useState)("");
  const [newColumnColor, setNewColumnColor] = (0, import_react11.useState)("#6B7280");
  const [bulkMode, setBulkMode] = (0, import_react11.useState)(false);
  const [selectedTasks, setSelectedTasks] = (0, import_react11.useState)(/* @__PURE__ */ new Set());
  const [showBulkActions, setShowBulkActions] = (0, import_react11.useState)(false);
...
//# sourceMappingURL=/chunks/chunk-GPQITWN7.js.map
